...
public class CurrConvExchangeService implements ExchangeService {
    ...
    protected BigDecimal fetchExchangeRateFor(String currencyFrom,String currencyTo) {
    ...
    private BigDecimal extractExchangeRate(String response){
        int columIndex=response.lastIndexOf(':');
        int closingBracket=response.lastIndexOf('}');
        String exchangeRate=response.substring(columIndex+1,closingBracket);
